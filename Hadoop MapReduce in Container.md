...
You'll do this inside the same Hive container from `hiveInstructions.md`, since it already bundles a working Hadoop installation — you don't need a separate Hadoop setup.
...
11. If you haven't touched anything yet, skip this step. Otherwise: press `ESC` to leave whatever mode you're in, then type `:q!` and press Enter to quit without saving — worth remembering any time you want to bail out of vim without keeping changes.

12. Press the `Insert` key (or `i`) to enter INSERT mode, which lets you type and edit text normally.

13. Paste the following into `mapper.py`. This script reads text line by line from standard input, splits each line into words, and prints each word out paired with the number `1` — the raw material the reducer will later sum up:
```python
#!/usr/bin/env python3
"""mapper.py"""
...
19. Now chain the mapper's output through `sort` (to group identical words together, mimicking what Hadoop does automatically) and into the reducer. You should get back a word count — `foo` appearing 3 times, `quux` 2 times, and `labs`/`bar` once each:
```
