...
Hive itself needs Hadoop underneath it to store and process data (that's what "data warehousing on top of Hadoop" means), but the `apache/hive` container image already bundles a compatible Hadoop distribution for you — you don't need to install Hadoop separately for this exercise.
...
2. Open PowerShell (or the terminal of your choice) and start the Podman virtual machine. This step is only needed on Windows and macOS — Podman runs containers inside a small Linux VM on those platforms. On Linux, Podman talks to the OS directly, so you can skip this command:
```
...
6. Once the build finishes, start a container from that image. This launches Hive with its built-in Derby database as the metastore (fine for learning/testing, but not for production use — Derby doesn't handle concurrent access well), and exposes two ports: 10000 for JDBC clients like beeline, and 10002 for the web UI:
```
...
8. Give HiveServer2 a few seconds to finish starting up in the background before connecting — otherwise beeline may fail to connect on the first try. Then open an interactive shell inside the running container:
```
...
10. Now that you're in the beeline prompt, create a table. This defines a table named `hive_example` with two columns — a string column `a` and an integer column `b`:
```sql
...
12. You should see `hive_example` in the output. Now inspect it in more detail — this shows metadata like column types and, importantly, the HDFS/warehouse location where Hive is physically storing the table's data:
```sql
DESCRIBE FORMATTED hive_example;
```

13. Insert a few rows and run some basic aggregate queries against them. (Note: this uses the same table name, `hive_example`, that you created in step 10 — a common mistake is inserting into a differently-named table that was never created, which will fail.)
```sql
...
14. To leave beeline, use its built-in quit command (not `ctrl+c`, which just kills your terminal session ungracefully):
```
!quit
```
